...
export function refreshUserBadge() {
  ...
  if (badge && user) {
    const displayName = user.full_name || user.role || 'User';
    if (document.body?.dataset?.role === 'ipid') {
      const initials = displayName
        ...
        .join('') || 'IP';
      badge.innerHTML = `<span class="ipid-user-avatar">${initials}</span><span class="ipid-user-copy"><strong>${displayName}</strong><small>IPID REVIEWER</small></span>`;
    } else {
      badge.textContent = `${displayName} • ${user.role}`;
    }
  }
}
