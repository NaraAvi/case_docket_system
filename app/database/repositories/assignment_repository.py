...
class AssignmentRepository(BaseSqlAlchemyRepository):
    ...
    def query_by_officer(self, officer_id):
        with self._app_context():
            instances = self.session.query(self.model).filter_by(officer_id=officer_id).all()
    ...
    def end_assignment(self, assignment_id, ended_by=None, ended_by_role=None, reason=None):
        with self._app_context():
            instance = self.session.query(self.model).filter_by(assignment_id=assignment_id).first()
            if instance is None:
                return None
    ...
    def list_suspended_for_case(self, case_reference):
        with self._app_context():
            instances = self.session.query(self.model).filter_by(case_reference=case_reference, status="SUSPENDED").all()
        matches = [self._serialize(instance) for instance in instances]
        matches.sort(key=lambda item: str(item.get("assigned_at") or ""))
        return matches
    ...
    def suspend_assignment(self, assignment_id, suspended_by=None, reason=None):
        with self._app_context():
            instance = self.session.query(self.model).filter_by(assignment_id=assignment_id, status="ACTIVE").first()
            if instance is None:
                return None
    ...
    def reinstate_assignment(self, assignment_id):
        with self._app_context():
            instance = self.session.query(self.model).filter_by(assignment_id=assignment_id, status="SUSPENDED").first()
            if instance is None:
                return None
